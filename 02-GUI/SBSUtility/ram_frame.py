...
class ram_frame(LabelFrame):
	...
	def __init__(self,window,protocol,trace,id):
		...
		self.gui_spacer("---")
		self.gui_entry("Goal Position", "goal_position", 0, True, True, True, 0x83, 1, 2 )
		...
		self.gui_spacer("---")
		self.gui_entry("Present Position", "present_position", 0, False, True, False, 0x90, 1, 2 )
		...
		self.gui_spacer("---")
		self.gui_entry("Setpoint Position", "setpoint_position", 0, False, True, False, 0xA0, 1, 2 )
	...
	def read_all(self):
		...
		if error != 0 :
			...
			goal_position 		= float(sign(result[3] + (result[4]<<8)))
			setpoint_position 	= float(sign(result[32] + (result[33]<<8)))
			present_position 	= float(sign(result[16] + (result[17]<<8)))
